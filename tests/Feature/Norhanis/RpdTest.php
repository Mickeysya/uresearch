...
class RpdTest extends TestCase
{
    ...
    public function test_the_ceiling_caps_what_a_student_may_request(): void
    {
        ...
        $candidacy->update(['extension_months_used' => 4]);

        $this->actingAs($candidacy->student);

        // 3 months asked, 2 left under the six-month ceiling.
    ...
    public function test_faculty_final_approval_closes_the_candidacy_and_emails_the_student(): void
    {
        Notification::fake();
        ...
        // Registry is not a stage: it cannot decide, at any point in the chain.
        $this->actingAs($this->registry())
            ->post(route('rpd-dismissal.decide', $application), ['decision' => 'approve'])
            ->assertForbidden();

        foreach ([$this->dean(), $this->faculty()] as $approver) {
            $this->actingAs($approver)
                ->post(route('rpd-dismissal.decide', $application), ['decision' => 'approve'])
                ->assertRedirect();
        }
        ...
        // norhanis.md 4.3: Registry's official notice follows Faculty's approval, so
